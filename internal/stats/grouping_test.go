...
func TestComputeTurnTokenMetricsByVersionGroupsSelectedProvider(t *testing.T) {
	t.Parallel()

	now := time.Date(2026, 4, 14, 12, 0, 0, 0, time.UTC)

	got := ComputeTurnTokenMetricsByVersion(
		[]SessionTurnMetrics{
			{
	...
	assert.Equal(t, UnknownVersionLabel, got[1].Version)
